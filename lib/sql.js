...
SQLConnector.prototype.buildUpdate = function(model, where, data, options) {
  var fields = this.buildFieldsForUpdate(model, data, undefined, options);
 ...
 */
SQLConnector.prototype.buildFieldsForUpdate = function(model, data, excludeIds, options) {

  /*
  * passed options object, to enable decrement/increment
  * - options.decrementField {string}
  * - options.decrementLimit | options.incrementLimit {number, optional}
  * */
  ...
  for (var i = 0, n = fields.names.length; i < n; i++) {
    ...
    /* transform default query placeholder '?' to make SQL increment/decrement */
    if (options) {
      // if decremented property defined && matches
      if (options.decrementField && this.columnEscaped(model, options.decrementField) === fields.names[i]) {
        // if decrement limit defined
        if (typeof options.decrementLimit === 'number') {

          // example: IF( `qtyInStock` - ? <= 0, 0, `qtyInStock` - ? )
          fields.columnValues[i].sql = `IF( ${fields.names[i]} - ${fields.columnValues[i].sql} <= ${options.decrementLimit}, ${options.decrementLimit}, ${fields.names[i]} - ${fields.columnValues[i].sql} )`;

          // copy param value for second '?'; example: [2].unshift(2)
          var tmpParams = fields.columnValues[i].params;
          tmpParams.push( tmpParams[tmpParams.length - 1] )

        } else {
          // example: ( `qtyInStock` - ? )
          fields.columnValues[i].sql = `( ${fields.names[i]} - ${fields.columnValues[i].sql} )`;
        }
      // else if incremented property defined && matches
      } else if (options.incrementField && this.columnEscaped(model, options.incrementField) === fields.names[i]) {
        // if increment limit defined
        if (typeof options.incrementLimit === 'number') {

          // example: IF( `qtyInStock` + ? >= 999, 999, `qtyInStock` + ? )
          fields.columnValues[i].sql = `IF( ${fields.names[i]} + ${fields.columnValues[i].sql} >= ${options.incrementLimit}, ${options.incrementLimit}, ${fields.names[i]} + ${fields.columnValues[i].sql} )`;

          // copy param value for second '?'; example: [5].unshift(5)
          var tmpParams = fields.columnValues[i].params;
          tmpParams.push( tmpParams[tmpParams.length - 1] )
...
SQLConnector.prototype.buildCounts = function (model, data) {
  var sCountsQuery = '',
      aCountParams = [],
      self = this;

  data.forEach(function(oData, iIndex) {
    var oSettings = oData.settings
        , oWhereSmtp = null
        , sFromTable
        , sWhereColumn
        , sWhereForRelation = '';
    ...
    if (oSettings.type === 'hasAndBelongsToMany') {
      sFromTable = model + oSettings.model;
      sWhereColumn = self.columnEscaped(oSettings.model, oSettings.foreignKey);
    } else if(oSettings.type === 'hasMany' && oSettings.through){
      sFromTable = oSettings.through;
      sWhereColumn = self.columnEscaped(oSettings.through, model + 'Id');
    } else {
      sWhereColumn = self.columnEscaped(oSettings.model, oSettings.foreignKey);
      sFromTable = oSettings.model;
    }

    sWhereForRelation = 'outer_query.' + self.columnEscaped(model, 'id') + ' = inner_query_' + iIndex + '.' + sWhereColumn;

    if(oData.where)
      oWhereSmtp = self.buildWhere(sFromTable, oData.where);

    if(oWhereSmtp)
      oWhereSmtp.sql += ' AND ' + sWhereForRelation;
    else
      oWhereSmtp = ParameterizedSQL('WHERE ' + sWhereForRelation, []);

    if(sCountsQuery.length)
      sCountsQuery += ', ';

    sCountsQuery += '(SELECT COUNT(*) FROM ' + self.tableEscaped(sFromTable) + ' AS inner_query_' + iIndex + ' ' + oWhereSmtp.sql + ') AS ' + oSettings.name + 'Count';

    aCountParams = aCountParams.concat(oWhereSmtp.params)

  });

  return ParameterizedSQL(sCountsQuery, aCountParams)
};
