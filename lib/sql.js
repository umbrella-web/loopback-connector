...
SQLConnector.prototype._buildWhere = function(model, where) {
  ...
  var oThrough //get through filter
      , oSelf //get self filter
      , oOther = {}; //save other data

  //first save all data to associated vars
  ...
  //check dirty other data, if empty, then del
  ...
  //if through and self not found, then self = where
  ...
  if(oOther) {
    if(typeof oSelf === 'object')
      for(var k in oOther)
        oSelf[k] = oOther[k];
    else //if self not found, but through find, then we have other data
      oSelf = oOther;
  }
  ...
  //save all stmts(sql + params) for through
  var throughStmts = [];
  if(typeof oThrough === 'object') {
    for (var sRel in oThrough) { //sRel = relation name
      var throughProps = self.getModelDefinition(oThrough[sRel].params.model).properties
          , throughInnerStmts = []; //save all stmt for relation through { items: {}, invoices: {} } (items, invoices - relations, and they contain where filter)
      for (var sKey in oThrough[sRel].where) {
        throughInnerStmts.push({ //each operator in where filter (or, and) convert to sql + params
  ...
  throughStmts.forEach(function(aStmts){ //all inner sql for relations must combine
    var sqls = []
        , stmtParams = [];
