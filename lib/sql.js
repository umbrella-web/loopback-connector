    ...
    if (options) {
      ...
      if (options.decrementField && this.columnEscaped(model, options.decrementField) === fields.names[i]) {
        ...
        if (typeof options.decrementLimit === 'number') {
          fields.columnValues[i].sql = `IF( ${fields.names[i]} <= ${options.decrementLimit}, ${options.decrementLimit}, ${fields.names[i]} - ${fields.columnValues[i].sql} )`;
        } else {
      ...
      // else if incremented property defined && matches
        ...
        if (typeof options.incrementLimit === 'number') {
          fields.columnValues[i].sql = `IF( ${fields.names[i]} >= ${options.incrementLimit}, ${options.incrementLimit}, ${fields.names[i]} + ${fields.columnValues[i].sql} )`;
        } else {
          fields.columnValues[i].sql = `( ${fields.names[i]} + ${fields.columnValues[i].sql} )`;
        }
      }
