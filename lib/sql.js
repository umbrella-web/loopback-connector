...
SQLConnector.prototype.buildCounts = function (model, props) {
  ...
  props.forEach(function(oProp, iIndex) {
      ...
      if (oProp.relationType === 'hasAndBelongsToMany') {
        sFromTable = model + oProp.modelToName;
      } else {
        sFromTable = oProp.modelToName;
      }
      ...
      sCountsQuery += ', (SELECT COUNT(*) FROM ' + self.tableEscaped(sFromTable) + ' AS inner_query_' + iIndex
        ...
        + ' = '
        + 'inner_query_' + iIndex + '.' + self.columnEscaped(oProp.modelToName, oProp.keyTo) + ') AS ' + oProp.relationName + 'Count'
  });
