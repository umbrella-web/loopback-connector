...
SQLConnector.prototype._buildWhere = function(model, where) {
  ...
  //where = typeof where === 'object' && typeof where.self === 'object' && where.self || where;
  var oThroughWhere = {};
  if(typeof where === 'object' && where.through) {
    oThroughWhere = where.through;
    delete where.through;
  }

  if(typeof where === 'object' && where.self)
    where = where.self;
  ...
  for (var key in where) {
    whereStmts.push(stmt.call(this, where[key], key, props));
  }

  var whereThroughtStmts = [];
  for (var sRel in oThroughWhere) {
    var throughProps = self.getModelDefinition(oThroughWhere[sRel].params.model).properties
        , whereThroughtInnerStmts = [];
    for(var sKey in oThroughWhere[sRel].where) {
      whereThroughtInnerStmts.push({
        stmt: stmt.call(this, oThroughWhere[sRel].where[sKey], sKey, throughProps),
        params: oThroughWhere[sRel].params
      });
    }

    if(whereThroughtInnerStmts.length) {
      whereThroughtStmts.push(whereThroughtInnerStmts);
    }
  }

  var aStructuredThroughtStmts = [];
  whereThroughtStmts.forEach(function(aStmts){
    var sqls = []
        , stmtParams = [];
    ...
    aStructuredThroughtStmts.push(stmt);
  ...
  whereStmts = whereStmts.concat(aStructuredThroughtStmts);
