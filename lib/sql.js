...
SQLConnector.prototype.buildCounts = function (model, data) {
  ...
  data.forEach(function(oData, iIndex) {
    ...
    if(oData.where)
      oWhereStmt = self.buildWhere(sFromTable, oData.where);

    if(oWhereStmt)
      oWhereStmt.sql += ' AND ' + sWhereForRelation;
    else
      oWhereStmt = ParameterizedSQL('WHERE ' + sWhereForRelation, []);

    if(sCountsQuery.length)
      sCountsQuery += ', ';

    sCountsQuery += '(SELECT COUNT(*) FROM ' + self.tableEscaped(sFromTable) + ' AS inner_query_' + iIndex + ' ' + oWhereStmt.sql + ') AS ' + oSettings.name + 'Count';

    aCountParams = aCountParams.concat(oWhereStmt.params)
