...
SQLConnector.prototype._buildWhere = function(model, where) {
  ...
  var oThrough //get through filter
      , oSelf
      , oOther = {};
  if(typeof where === 'object') {

    if (where.through) {
      oThrough = where.through;
      delete where.through;
    }

    if (where.self) { //get self filter
      oSelf = where.self;
      delete where.self;
    }
  ...
  var bOtherDirty = false;
  for(var kO in oOther) {
    bOtherDirty = true;
    break;
  }
  if(!bOtherDirty)
  ...
  if (typeof oSelf !== 'object' || Array.isArray(oSelf)) {
    debug('Invalid value for where: %j', oSelf);
    return new ParameterizedSQL('');
  }
  var self = this;
  ...
  var selfStmts = [];
  for (var key in oSelf) {
    selfStmts.push(stmt.call(this, oSelf[key], key, props));
  }

  var throughStmts = [];
  if(typeof oThrough === 'object') {
    for (var sRel in oThrough) {
      var throughProps = self.getModelDefinition(oThrough[sRel].params.model).properties
          , throughInnerStmts = [];
      for (var sKey in oThrough[sRel].where) {
        throughInnerStmts.push({
          stmt: stmt.call(this, oThrough[sRel].where[sKey], sKey, throughProps),
          params: oThrough[sRel].params
        });
      }

      if (throughInnerStmts.length) {
        throughStmts.push(throughInnerStmts);
      }
    }
  }

  var aStructuredThroughtStmts = [];
  throughStmts.forEach(function(aStmts){
    var sqls = []
        , stmtParams = [];
    for (var k = 0, s = aStmts.length; k < s; k++) {

      var params = aStmts[k].params
          , stmt = aStmts[k].stmt;

      stmt.sql = self.columnEscaped(model, 'id') + ' IN ' +
          '(SELECT ' + self.columnEscaped(params.through, model + 'Id') + ' FROM ' + params.through + ' WHERE ' + self.columnEscaped(params.through, params.model + 'Id') + ' IN ' +
          '(SELECT ' + self.columnEscaped(params.through, 'id') + ' FROM ' + self.tableEscaped(params.model) + ' WHERE '+ stmt.sql +' ))';

      sqls.push(stmt.sql);
      stmtParams = stmtParams.concat(stmt.params);
    }
    var throughStmt = new ParameterizedSQL({
      sql: sqls.join(' AND '),
      params: stmtParams
    });

    aStructuredThroughtStmts.push(throughStmt);

  });

  function stmt(clausesNexpression, key, props) {
    var stmt = new ParameterizedSQL('', []);
    // Handle and/or operators
    if (key === 'and' || key === 'or') {
      var branches = [];
      var branchParams = [];
      var clauses = clausesNexpression;
    ...
    var expression = clausesNexpression;
    var columnValue;
    var sqlExp;
  ...
  var allStmts = selfStmts.concat(aStructuredThroughtStmts);

  for (var k = 0, s = allStmts.length; k < s; k++) {
    sqls.push(allStmts[k].sql);
    params = params.concat(allStmts[k].params);
  }
...
SQLConnector.prototype.buildCounts = function (model, props) {
  var sCountsQuery = '',
      self = this,
      sFromTable,
      sWhereColumn;

  props.forEach(function(oProp, iIndex) {
    // hasAndBelongsToMany relation uses table with composite name
    if (oProp.type === 'hasAndBelongsToMany') {
      sFromTable = model + oProp.model;
      sWhereColumn = self.columnEscaped(oProp.model, oProp.foreignKey);
    } else if(oProp.type === 'hasMany' && oProp.through){
      sFromTable = oProp.through;
      sWhereColumn = self.columnEscaped(oProp.through, model + 'Id');
    } else {
      sWhereColumn = self.columnEscaped(oProp.model, oProp.foreignKey);
      sFromTable = oProp.model;
    }

    sCountsQuery += ', (SELECT COUNT(*) FROM ' + self.tableEscaped(sFromTable) + ' AS inner_query_' + iIndex
        + ' WHERE outer_query.' + self.columnEscaped(model, 'id')
        + ' = '
        + 'inner_query_' + iIndex + '.' + sWhereColumn + ') AS ' + oProp.name + 'Count'
...
/*SQLConnector.prototype.buildThroughWhere = function (model, props) {
 var bFirstWhere = true
 , oThroughWhereStmt = new ParameterizedSQL();
 for(var k in props) {
 var oParams = props[k].params
 , oWhere = props[k].where
 , sFromTable = oParams.through
 , sWhereColumn = this.columnEscaped(oParams.through, model + 'Id')
 , oThroughWhereSql = this.buildWhere(oParams.model, oWhere);

 oThroughWhereSql.sql = (bFirstWhere ? 'WHERE ' : ' AND ') + this.columnEscaped(model, 'id') + ' IN ' +
 '(SELECT ' + this.columnEscaped(oParams.through, model + 'Id') + ' FROM ' + oParams.through + ' WHERE ' + this.columnEscaped(oParams.through, oParams.model + 'Id') + ' IN ' +
 '(SELECT ' + this.columnEscaped(oParams.through, 'id') + ' FROM ' + this.tableEscaped(oParams.model) + ' '+ oThroughWhereSql.sql +' ))';

 oThroughWhereStmt.merge(oThroughWhereSql);
 bFirstWhere = false;
 }
 return oThroughWhereStmt;
 }*/
...
SQLConnector.prototype.buildSelect = function(model, filter, options) {
  ...
  filter && filter.count && (sCountSql = this.buildCounts(model, filter.count));

  var selectStmt = new ParameterizedSQL('SELECT ' +
      this.buildColumnNames(model, filter) +
      ((sCountSql.length && sCountSql) || '') +
      ' FROM ' + this.tableEscaped(model) + ' outer_query'
  );
  ...
  if (filter) {
    ...
    /*if(filter.throughWhere) {

     var throughWhereStmt = this.buildThroughWhere(model, filter.throughWhere);
     if(whereStmt.sql && throughWhereStmt.sql)
     throughWhereStmt.sql = 'AND' + throughWhereStmt.sql.substring(5); //CHANGE WHERE - AND

     selectStmt.merge(throughWhereStmt);
