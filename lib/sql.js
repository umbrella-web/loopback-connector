...
SQLConnector.prototype.buildSelect = function(model, filter, options) {
  ...
  if (filter) {
    ...
    if(filter.throughWhere) {
      for(var key in filter.throughWhere) {
        var oParams = filter.throughWhere[key].params
            , oWhere = filter.throughWhere[key].where
            , oThroughWhereSql = this.buildWhere(oParams.model, oWhere);

        oThroughWhereSql.sql = (bFirstWhere ? 'AND ' : 'WHERE ') + this.columnEscaped(model, 'id') + ' IN ' +
        ...
        selectStmt.merge(oThroughWhereSql);
        bFirstWhere = true;
      }
    }
